      ...
      namespace user
      {
         ...
         void display::open_display()
         {
            ...
            if (platformName == "wayland") {
               debug() << "The display server is Wayland.";
               initialize_display_type(e_display_type_wayland);
            } else if (platformName == "xcb") {
               debug() << "The display server is X11.";
               initialize_display_type(e_display_type_xcb);
            } else {
               debug() << "Unknown display server: " << platformName;
               initialize_display_type(e_display_type_none);
            }
         ...
         void display::kick_idle()        // if (m_pconnection != nullptr)
            // {
            //
            //    xcb_disconnect(m_pconnection);
            //
         ...
         ::size_i32 display::get_main_screen_size()
         {
            ...
            auto *pqscreen = QGuiApplication::primaryScreen();

            if (!pqscreen)
            {

               debug() << "No screen detected!";

               debug() << "Returning default size of Full HD 1920x1080!";

               return {1920, 1080}; // return some default size
               // Get the screen size
            }

            auto qsize = pqscreen->size();
