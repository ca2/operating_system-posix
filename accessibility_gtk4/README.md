# accessibility_gtk4 for FreeBSD

This directory is a FreeBSD/GTK4 replacement for the copied
`accessibility_windows` skeleton.

## What changed

- Renamed the CMake project, factory and C++ namespace to `accessibility_gtk4`.
- Removed dependencies on `acme_windows`, Win32 resources, Shell API, GDI+,
  SetupAPI and CredUI.
- Uses `pkg-config`'s `gtk4` target, which supplies GTK, GDK, GLib, GObject,
...
- Adds GTK4-native helpers for accessible labels, descriptions, ranges,
...
```text
/home/camilo/code/main/operating_system/operating_system-freebsd/accessibility_gtk4
```
...
```cmake
set(ACCESSIBILITY_GTK4_CA2_LIBRARIES axis acme acme_posix)
add_subdirectory(accessibility_gtk4)
```

## GTK widget use

```cpp
#include "accessibility_gtk4/gtk_accessible.h"

accessibility_gtk4::set_accessible_label(pwidget, "Open document");
accessibility_gtk4::set_accessible_description(
   pwidget,
   "Opens an existing document from disk");

accessibility_gtk4::set_accessible_disabled(pwidget, false);
accessibility_gtk4::set_accessible_busy(pwidget, true);

accessibility_gtk4::announce_accessible_message(
   pwidget,
   "Document saved");
```
...
those missing files. The GTK4 helper implementation itself is complete.
