...
namespace windows
{
   ...
   public:
	   ...
	   virtual bool AddMappingUsingThread( port_map& newMapping, ::windowing::window * pwindow );  // starts a thread that will add one __new mapping; the thread posts a UWM_PORT_FORWARD_ENGINE_THREAD_NOTIFICATION message to oswindow when it's done
