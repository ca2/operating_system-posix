...
////      {
////
////         informationf("----> Failed to createø < ::windowing::display > at windowing_xcb::windowing::initialize");
////
...
//            }
            ...
            else
            {
               ...
               ::i32_rectangle rectangleFoundFrameExtents;
...
//                  m_pSnLauncheeContext = nullptr;
            ...
            auto pmessage = create_newø<::message::show_window>();
         ...
         case XCB_CONFIGURE_NOTIFY:
         {
            ...
            ::i32_point point(pconfigure->x, pconfigure->y);

            ::i32_size size(pconfigure->width, pconfigure->height);
...
//                  }
         ...
         case XCB_BUTTON_RELEASE:
         {
            ...
            if (bRet && (!bHasTranslucency || bMouseCapture || u8Opacity != 0 || bTransparentMouseEvents))
            {
               ...
               if (Δ != 0)
               {
                  ...
                  auto pmessage = create_newø<::message::mouse_wheel>();
         ...
         case XCB_KEY_RELEASE:
         {
            ...
            if (bRet)
            {
               ...
               auto pmessage = create_newø<::message::key>();
               pmessage->m_pacmewindowingwindow = pxcbwindow;
               pmessage->m_pwindow = pxcbwindow;
               pmessage->m_eusermessage = emessage;
            ...
            if(pxcbwindow)
            {
               ...
               if (::is_set(pimpl))
               {
                  ...
                  if (::is_set(pinteraction))
                  {
                     ...
                     auto pmessage = create_newø<::message::set_keyboard_focus>();
            ...
            //if (::is_set(msg.oswindow))
            {
               ...
               if (::is_set(pimpl))
               {
                  ...
                  if (::is_set(pinteraction))
                  {
                     ...
                     auto pmessage = create_newø<::message::kill_keyboard_focus>();
...
//
//      auto pointWindow = ::i32_point(geometry.x, geometry.y);
//
//      auto sizeWindow = ::i32_size(geometry.width, geometry.height);
//
...
//         {
//
//            auto pmessage = create_newø<::message::reposition>();
//
...
//
//            pmessage->m_eusermessage = ::user::e_message_reposition;
//            pmessage->m_wparam = 0;
...
//         {
//
//            auto pmessage = create_newø<::message::size>();
//
//            pmessage->m_pwindow = oswindow;
//
//            pmessage->m_pacmewindowingwindow = oswindow;
...
//
//            pmessage->m_eusermessage = ::user::e_message_size;
//            pmessage->m_wparam = 0;
