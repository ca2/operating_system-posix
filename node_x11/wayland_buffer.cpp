...
//
   ...
   void wayland_buffer::_map_shared_memory(const ::i32_size & size)
   {
...
//   void wayland_buffer::_destroy_shared_memory()
//   {
//
//      if (m_shmaddr)
//      {
...
//   }
   ...
   ::windowing_wayland::window * wayland_buffer::x11_window()
   {

      return (::windowing_wayland::window *) (m_pwindow ? m_pwindow->m_pWindow4 : nullptr);
...
//      }
   ...
   bool wayland_buffer::update_buffer(::graphics::buffer_item * pbufferitem)
   {
...
//      }
   ...
   bool wayland_buffer::create_os_buffer(const ::i32_size & size, int iStrideParam)
   {
...
//
   ...
   void wayland_buffer::destroy_os_buffer()
   {
...
//   bool wayland_buffer::create_os_buffer(::image::image *pimage)
//   {
//
//      //synchronous_lock sl(synchronization(), DEFAULT_SYNCHRONOUS_LOCK_SUFFIX);
//
...
//   void wayland_buffer::destroy_os_buffer(::image::image *pimage)
//   {
//
//      if(m_pimage != nullptr)
//      {
...
//
   ...
   bool wayland_buffer::buffer_lock_round_swap_key_buffers()
   {
...
//
//   bool wayland_buffer::update_screen()
//   {
//
//      if (m_pimpl == nullptr)
//      {
//
//         warningf("windowing_wayland::wayland_buffer::update_screen !m_pimpl!!");
//
//         return false;
//
//      }
...
//      {
//
//         warningf("windowing_wayland::wayland_buffer::update_screen !m_pimpl->m_pwindow!!");
//
//         return false;
//
//      }
...
//      {
//
//         information() << "windowing_wayland::wayland_buffer::update_screen XPutImage not called. Ui is not visible.";
//
//         return false;
//
//      }
...
//      {
//
//         warningf("windowing_wayland::wayland_buffer::update_screen !m_pwindow!");
//
//         return false;
//
//      }
...
//   bool wayland_buffer::_update_screen_lesser_lock()
//   {
//
...
//   {
//      auto pbuffer = (wayland_buffer *) data;
...
//   };
   ...
   void wayland_buffer::__redraw(struct wl_callback *pwlcallback, uint32_t time)
   {
   ...
   void wayland_buffer::__handle_window_redraw(::wl_callback *pwlcallback, uint32_t time)
   {
...
//
//      }
////      wl_surface_attach(surface, wayland_buffer, 0, 0);
...
//   bool wayland_buffer::_update_screen_unlocked(::graphics::buffer_item * pitem)
//   bool wayland_buffer::_update_screen_unlocked(::graphics::buffer_item * pitem)
   //bool wayland_buffer::_post_update_screen()
   //{
   bool wayland_buffer::update_screen()
   {
...
//      {
      ...
      && pwaylandwindow->m_pxdgpopup == nullptr)
      {
         ...
         if(!pwaylandwindow->windowing()->is_screen_visible(edisplay) && edisplay != e_display_iconic)
         {
            ...
            information()
               << "::windowing_wayland::wayland_buffer::update_screen this is not visible";
         ...
         try {
            ...
            if (::is_null(pwaylandwindow->m_pxdgtoplevel)
               ...
               if (!pwaylandwindow->windowing()->is_screen_visible(edisplay) && edisplay != e_display_iconic) {
                  ...
                  information() << "::windowing_wayland::wayland_buffer::update_screen this is not visible (2)";
...
//                                       ::pointer<wayland_buffer> pbuffer = pimpl->m_pgraphics;
//
...
//                  ::minimum(pwaylandwindow->m_sizeConfigure.cy);
...
#ifdef MORE_LOG
         information() << "wayland_buffer::update_screen end";
#endif
      ...
      else
      {
         ...
         information() << "wayland_buffer::update_screen timeout";
...
//      {
//
//         warningf("windowing_wayland::wayland_buffer::update_screen X11 image null or empty!!");
//
//         return false;
//
//      }
...
//      {
//
//         warningf("windowing_wayland::wayland_buffer::update_screen m_gc nullptr!!");
//
//         return false;
//
//      }
//
//      ::i32_size sizeDesign = m_pimpl->m_puserinteraction->const_layout().design().size();
//
...
//      {
//
//         warning() << "What!! Drawn Buffer doesn't fit internal wayland_buffer (that should be at least same size as the wayland_buffer size)";
//
//      }
//
//      try
...
//
//      informationf("windowing_wayland::wayland_buffer::update_screen BASIC_TEST FillRectangle(%d, %d)", iWidth, iHeight);
//
//#endif
//
...
//      // }
   ...
   bool wayland_buffer::on_update_screen(::graphics::buffer_item * pitem)
   {
   ...
   bool wayland_buffer::_on_begin_draw(::graphics::buffer_item * pbufferitem)
   {
...
//   bool wayland_buffer::presentation_complete()
//   {
//
