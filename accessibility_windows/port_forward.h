...
namespace acme_windows
{
   ...
   public:
	   ...
	   {
		   IDerivedNATExternalIPAddressCallback( ::net::port_forward_change_callbacks * i32_point ) : m_pointer( i32_point ), m_dwRef( 0 ) { };
	   ...
	   {
		   IDerivedNATNumberOfEntriesCallback( ::net::port_forward_change_callbacks* i32_point ) : m_pointer( i32_point ), m_dwRef( 0 ) { };
	   ...
	   virtual bool AddMappingUsingThread( port_map& newMapping, ::windowing::window * pwindow );  // starts a thread that will add one ___new mapping; the thread posts a UWM_PORT_FORWARD_ENGINE_THREAD_NOTIFICATION message to oswindow when it's done
